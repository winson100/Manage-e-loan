...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function createFileItem(fileObj) {
        ...
        if (fileObj.status === 'uploading') {
            // 上传中显示进度圆圈
            iconHtml = `
                <div class="file-icon file-icon-uploading">
                    <div class="upload-circle">
                        <div class="upload-circle-progress" style="--progress: ${fileObj.progress}%"></div>
                        <span class="upload-percentage">${Math.round(fileObj.progress)}%</span>
                    </div>
                </div>
            `;
            ...
            // 上传成功显示身份证图片
            iconHtml = `
                <div class="file-icon file-icon-completed">
                    <img src="../assets/images/shenfenzheng.png" alt="身份证" class="file-icon-image">
                </div>
            `;
        } else {
            // 其他状态显示文件扩展名
            iconHtml = `<div class="file-icon">${fileExtension.toUpperCase()}</div>`;
        }
    ...
    const smsCodeInput = document.querySelector('.password-dots-row'); // 使用密码点输入容器
    const getSmsCodeBtn = null; // 获取验证码按钮不存在，设为null
    const closeSmsModalBtn = document.querySelector('.popup-title-icon-wrapper'); // 使用返回按钮
    ...
    const passwordInputs = document.querySelectorAll('.password-dots-row input');
    let currentSmsCode = '';
    ...
    if (deleteBtn) {
        deleteBtn.addEventListener('click', function() {
            if (currentSmsCode.length > 0) {
                currentSmsCode = currentSmsCode.slice(0, -1);
                updatePasswordDisplay();
                if (smsError) {
                    smsError.style.display = 'none';
                }
    ...
    function updatePasswordDisplay() {
        passwordInputs.forEach((input, index) => {
            if (index < currentSmsCode.length) {
                input.value = '*';
            } else {
                input.value = '';
            }
    ...
    function verifySmsCode() {
        ...
        if (code.length === 6) {
            ...
            setTimeout(() => {
                hideSmsModal();
                // 跳转到loan-result.html页面（与确定按钮的跳转页面一致）
                window.location.href = 'loan-result.html';
            }, 300);
        } else {
            // 验证失败（理论上不会执行到这里，因为只有输入满6位才会调用此函数）
            if (smsError) {
